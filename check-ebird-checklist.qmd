...
chk <- c %>% 
  mutate(
    ...
    chk_high_number_species = number_species>100,
    ...
    chk_too_many_observers = number_observers > 20,
...
For each check (edit the check name in the code below), you can also view the checklists flagged by observer.
...
In particular, there seems to be an issue with the start time of your checklist. Would you mind verifying your checklist?

Potential issues can be a confusion between AM/PM or a wrong start time. If no start time is available, the checklist protocol should be <a href=\"https://support.ebird.org/en/support/solutions/articles/48000950859-guide-to-ebird-protocols#anchorHistorical\">historical</a> if birding was a <a href=\"https://support.ebird.org/en/support/solutions/articles/48000967748-birding-as-your-primary-purpose-and-complete-checklists\">primary purpose</a> or <a href=\"https://support.ebird.org/en/support/solutions/articles/48000950859-guide-to-ebird-protocols#anchorIncidental\">incidental</a> if birding was not your primary purpose. In both cases, you can leave the time field empty. 

Could you please let me know if the time was originally correct, or if you have now corrected it, so that I can set the checklist as public again? 
...
I am writing about the following checklists(s): {checklists_link}

Your checklist(s) was flagged because they contain an unusually high number of species. We want to double check with you that the checklist follows <a href=\"https://support.ebird.org/en/support/solutions/articles/48000795623-ebird-rules-and-best-practices\">eBird rules and best practices</a>, in particular:
...
Could you please make the relevant corrections so that the checklist follows these guidelines, and let me know when done, so that I can set the checklist as public again?
...
email <- chk %>% 
  filter(chk_high_number_species) %>% 
  group_by(observer_id) %>%
  summarise(checklists_link = paste(checklist_link, collapse = ", "), number_checklist = n()) %>% 
  arrange(desc(number_checklist)) %>% 
  glue_data(template, .sep = "\n----\n",)

fileConn<-file("email/high_number_species.txt")
writeLines(email, fileConn)
close(fileConn)
```
...
Your checklist(s) was flagged because it as an usually high distance. While it is acceptable to make a checklist of up to 80km according to the <a href=\"https://support.ebird.org/en/support/solutions/articles/48000795623-ebird-rules-and-best-practices\">eBird rules and best practices</a>, more precise checklists are more valuable for science. It is therefore recommend to keep Traveling checklists under 5 miles (8 km) and Stationary checklists under 3 hours for your sightings to make the biggest impact for science. Limiting your checklists to less than one hour or one mile provides even more checklist precision! 

Please do not submit one checklist spanning several locations, especially when those locations were separated by long drives in between. A good rule of thumb is: every time you get in a car, stop the previous list and start a new list at the next location.

In light of the above, could you please check that the location, duration, distance and protocol of your checklist are correct, and let me know when done, so that I can set the checklist as public again?
...
fileConn<-file("email/too_long_distance.txt")
writeLines(email, fileConn)
close(fileConn)
```
...
Could you please make the appropriate correction and let me know when done, so that I can set the checklist as public again?
...
email <- chk %>% 
  filter(chk_multi_day) %>% 
  group_by(observer_id) %>%
  summarise(checklists_link = paste(checklist_link, collapse = ", "), number_checklist = n()) %>% 
  arrange(desc(number_checklist)) %>% 
  glue_data(template, .sep = "\n----\n",)

fileConn<-file("email/multi_day.txt")
writeLines(email, fileConn)
close(fileConn)
```
...
Your checklist(s) was flagged because it is a complete checklist with only a single species. While this might be correct, we wanted to double check with you that birding was indeed your primary purpose and you recorded all the birds around you to the best of your abilities. (Please refer to the <a href=\"https://support.ebird.org/en/support/solutions/articles/48000967748-birding-as-your-primary-purpose-and-complete-checklistss\">eBird support help page</a> for more information).

If not, you can change the protocol to 'Incidental' or mark your list as 'Incomplete'.

Could you please let me know when you have made the appropriate correction so I can set the checklist as public again?
...
email <- chk %>% 
  filter(chk_only_one_species) %>% 
  group_by(observer_id) %>%
  summarise(checklists_link = paste(checklist_link, collapse = ", "), number_checklist = n()) %>% 
  arrange(desc(number_checklist)) %>% 
  glue_data(template, .sep = "\n----\n",)

fileConn<-file("email/only_one_species.txt")
writeLines(email, fileConn)
close(fileConn)
```

### Same species as media
...
email <- chk %>% 
  filter(chk_complete_media) %>% 
  group_by(observer_id) %>%
  summarise(checklists_link = paste(checklist_link, collapse = ", "), number_checklist = n()) %>% 
  arrange(desc(number_checklist)) %>% 
  glue_data(template, .sep = "\n----\n",)

fileConn<-file("email/complete_media.txt")
writeLines(email, fileConn)
close(fileConn)
```

### Same count all species
...
Your checklist(s) was flagged because all species were recorded with the same count. We wanted to double check with you that these counts are indeed correct (see more on <a href=\"https://support.ebird.org/en/support/solutions/articles/48000838845\">how to count birds with eBird</a>).

If you did not count the birds, you can udpate the count to \"X\".
...
email <- chk %>% 
  filter(chk_same_count_all_species) %>% 
  group_by(observer_id) %>%
  summarise(checklists_link = paste(checklist_link, collapse = ", "), number_checklist = n()) %>% 
  arrange(desc(number_checklist)) %>% 
  glue_data(template, .sep = "\n----\n",)

fileConn<-file("email/same_count_all_species.txt")
writeLines(email, fileConn)
close(fileConn)
```
...
Can you check that the distance is not a typo (e.g, 0.01 instead of 0.1) or change the protocol to Stationary?

Please let me know when you have made the appropriate correction so I can set the checklist as public again.
...
email <- chk %>% 
  filter(chk_not_traveling) %>% 
  group_by(observer_id) %>%
  summarise(checklists_link = paste(checklist_link, collapse = ", "), number_checklist = n()) %>% 
  arrange(desc(number_checklist)) %>% 
  glue_data(template, .sep = "\n----\n",)

fileConn<-file("email/traveling.txt")
writeLines(email, fileConn)
close(fileConn)
```
...
Your checklist(s) was flagged because we suspect that the Stationary protocol is incorrectly used. Can you check the <a href=\"https://support.ebird.org/en/support/solutions/articles/48000950859-guide-to-ebird-protocols#anchorStationary\">guide to eBird protocol</a> and adjust the protocol as necessary? If you're unable to estimate the distance covered, the Historical protocol might be the most adapted. 
...
email <- chk %>% 
  filter(chk_not_stationary) %>% 
  group_by(observer_id) %>%
  summarise(checklists_link = paste(checklist_link, collapse = ", "), number_checklist = n()) %>% 
  arrange(desc(number_checklist)) %>% 
  glue_data(template, .sep = "\n----\n",)

fileConn<-file("email/high_not_stationary.txt")
writeLines(email, fileConn)
close(fileConn)
```
...
email <- chk %>% 
  filter(chk_too_short_duration) %>% 
  group_by(observer_id) %>%
  summarise(checklists_link = paste(checklist_link, collapse = ", "), number_checklist = n()) %>% 
  arrange(desc(number_checklist)) %>% 
  glue_data(template, .sep = "\n----\n",)

fileConn<-file("email/too_short_duration.txt")
writeLines(email, fileConn)
close(fileConn)
```
...
Your checklist(s) was flagged because we suspect a potential issue with distance and duration entered. Could you double check the values entered and verify the validity of the protocol selected according to the <a href=\"https://support.ebird.org/en/support/solutions/articles/48000795623-ebird-rules-and-best-practices\">eBird rules and best practices</a>?
...
fileConn<-file("email/too_fast.txt")
writeLines(email, fileConn)
close(fileConn)
```
